...
export const CareerTableRow = ({ career, onOpenModal }: CareerTableRowProps) => {
  return (
    <tr className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group">
      <td className="sticky left-0 z-10 bg-white dark:bg-surface-dark group-hover:bg-slate-50 dark:group-hover:bg-slate-800 px-6 py-4 whitespace-nowrap text-slate-900 dark:text-white font-medium border-r border-slate-200 dark:border-slate-700">
      ...
      <td className="px-6 py-4 text-slate-600 dark:text-slate-400 max-w-xs truncate" title={career.description}>
        {career.description}
      </td>
      <td className="px-6 py-4 whitespace-nowrap">
      ...
      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
        <button className="text-primary hover:text-blue-400 transition-colors">Edit</button>
      </td>
    </tr>
  );
};
